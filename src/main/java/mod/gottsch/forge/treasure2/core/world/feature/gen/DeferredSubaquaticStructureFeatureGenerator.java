...
/**
 *
 * @author Mark Gottschling July 27, 2024
 *
 */
public class DeferredSubaquaticStructureFeatureGenerator implements IFeatureGenerator {

	private ResourceLocation name = new ResourceLocation(Treasure.MODID, "deferred_subaquatic_structure");
	...
	public Optional<GeneratorResult<ChestGeneratorData>> generate(IFeatureGenContext context, ICoords spawnCoords,
		...
		context.level().setBlock(spawnCoords.toPos(), TreasureBlocks.DEFERRED_SUBAQUATIC_GENERATOR.get().defaultBlockState(), 3);
		DeferredSubaquaticGeneratorBlockEntity be = (DeferredSubaquaticGeneratorBlockEntity) context.level().getBlockEntity(spawnCoords.toPos());
		if (be == null) {
			return Optional.empty();
		}
		be.setRarity(rarity);
